...
def moments(series: pd.Series) -> (float, float, float, float, float):
    ...
    return mu, sigma, skew, kurt, excess_kurt
...
def create_panel_df(series: pd.Series) -> pd.DataFrame:
    ...
    indexing = panel_data[-1].index.floor('D').strftime('%Y-%m-%d')
...
def dickey_fuller_test(data:pd.Series, confidence_interval: float = 0.05):
...
def adf_test_result(data:pd.Series, confidence_interval: float=0.05):
...
def kpss_test_result(series:pd.Series, confidence_interval:float = 0.05) -> bool:
...
def hurst_exponent_test(series: pd.Series) -> float:
    
    hurst = compute_Hc(series)[0]
    
    mean_reversion = hurst < 0.5
    random_walk = hurst == 0.5
    trending = hurst > 0.5
    
    print('Hurst Exponent Test')
    print('='*80)
    print('The Hurst Exponent for the given sample is {:.2f}'.format(hurst))
    print('-'*80)
    if mean_reversion:
        print('The given sample shows mean reversion')
        print('='*80)
    elif random_walk:
        print('The given sample is a random walk')
        print('='*80)
    elif trending:
        print('The given sample shows a persistent behaviour')
        print('='*80)

        
def hurst_test_result(series: pd.Series) -> str:
...
class WaveletApproximation():
    ...
    def predict(self, X):
        ...
        value_range = X #pd.Series(date_parser(X.index), index = X.index)
    ...
    def wavelets(self, X, sigma):
        ...
        value_range = np.array(X) #pd.Series(date_parser(X.index), index = X.index)
        multiplier = self.multiplier
        frequencies = self.frequencies
...
def compute_metrics(actual: np.array, forecast: np.array) -> (float, float, float):
...
def print_performance_metrics(actual: np.array, forecast: np.array) -> float:
    
    mae, mse, rmse = compute_metrics(actual, forecast)
